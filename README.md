...
- **Sim, animations, and UI decoupled from FPS.** Every frame-counted timer in the original engine — simulation ticks, tile animations, character walk/idle frames, UI blink cycles — has been converted to wall-clock pacing. The game plays at correct speed at any frame rate, so the FPS cap can be raised or removed without affecting gameplay. Two new `towns.ini` keys: `FPS_CAP` (default `0` = unlimited; positive integer = hard FPS cap; menu and in-game both follow it) and `VSYNC` (default `true`; tearing prevention; set `false` to remove the soft cap from the display refresh rate).
